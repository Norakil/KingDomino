...
{
	protected ArrayList<String>  allRois = new ArrayList<String>();	// Rois pour ordre de départ
	
	private int			nbRois			;									// Nombre de Rois
	...
	public KingDomino()
	{
		...
		// Ajouts des Rois
		this.allRois.add( "Bleu"	);
		this.allRois.add( "Rouge"	);
		this.allRois.add( "Bleu"	);
		this.allRois.add( "Rouge"	);
		...
		Collections.shuffle( this.allRois );
	}
	
	public void changementJoueur( String Roi )
	{
		if ( Roi.equals( "Bleu" ) )
			this.player = "Bleu";			// au tour du Joueur 1
		else if ( Roi.equals( "Rouge" ) )
			this.player = "Rouge";			// au tour du Joueur 2
	}
	...
	public String	getRoi1()		{ return this.allRois.get( 0 );	}
	public String	getRoi2()		{ return this.allRois.get( 1 );	}
	public String	getRoi3()		{ return this.allRois.get( 2 );	}
	public String	getRoi4()		{ return this.allRois.get( 3 );	}
	...
	public static void main (String[] args)
	{
		...
		PiocheDomino	pioche;
		Domino[]		ordre = new Domino[8];
		
		for ( int i = 0; i < ordre.length; i++ )
			ordre[i] = new Domino( null, null, 0, null );
		
		for ( int i = 0; i < 4; i++ )
			ordre[i].ajouterRoi( partie.allRois.get( i ) );
			
		for ( int i = 4; i < 8; i++ )
			ordre[i].ajouterRoi( partie.allRois.get( i-4 ) );
		...
		for ( int nbTours = 0; nbTours < 6; nbTours++ )
		{
			...
			for ( int i = 0; i < 4; i++ )
				ordre[i].ajouterRoi( ordre[i+4].getRoi() );
			...
			{
				partie.changementJoueur( ordre[cpt].getRoi() );
				...
				switch ( choix[cpt] )
				{
					case 1 :
						...
						ordre[4].ajouterRoi( pioche.getDomino( 0 ).getRoi() );
					break;
			
					case 2 :
						pioche.getDomino( 1 ).ajouterRoi( partie.getPlayer() );
						ordre[5].ajouterRoi( pioche.getDomino( 1 ).getRoi() );
					break;
			
					case 3 :
						pioche.getDomino( 2 ).ajouterRoi( partie.getPlayer() );
						ordre[6].ajouterRoi( pioche.getDomino( 2 ).getRoi() );
					break;
			
					case 4 :
						pioche.getDomino( 3 ).ajouterRoi( partie.getPlayer() );
						ordre[7].ajouterRoi( pioche.getDomino( 3 ).getRoi() );
					break;
				}
				System.out.println( pioche );
			}
			...
			for ( int cpt = 0; cpt < 4; cpt++ )
			{
				...
				System.out.println( pioche.getDomino( cpt ) );
			
				partie.changementJoueur( pioche.getDomino( cpt ).getRoi() );
				...
				partie.placerDomino( pioche.getDomino( cpt ), ligne, colonne, orientation );
				System.out.println( partie.afficherPlateau() );
			}
		}
